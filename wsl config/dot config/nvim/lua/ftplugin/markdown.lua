...
function compile()
	...
	local windows_path=string.upper(string.sub(linux_path,6,6))..':\\'..string.gsub(string.sub(linux_path,8),'/','\\')
	local pdf_file=string.sub(linux_path,#linux_path-string.find(string.reverse(linux_path),'/')+2,#linux_path-2)..'pdf'
	system('pandoc.exe -f markdown -t pdf "'..windows_path..'" > "'..pdf_file..'"')--compile
